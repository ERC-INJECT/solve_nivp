...
# The SOC Fischer-Burmeister primitives (_soc_fb_phi, _soc_fb_phi_and_jac,
# _soc_fb_phi_and_jac_2d, _soc_jordan_multiplication) now live in the leaf
# module ``soc`` and are imported above; they are kept importable from here
...
class ProjectedRadauContactModel:
    ...
    def _contact_residual(self, y, t, percussion, offset_measure, h, *, endpoint):
        ...
        for k, ci in enumerate(self.contacts):
            ...
            if velocity_normal and gaps[k] > self.gap_tol:
                # Moreau viability: an open contact is inactive, the law is
                # not invoked, and the reported reaction is driven to zero.
                out[sl] = effective_blk
                continue
            if velocity_normal:
    ...
    def _contact_jacobian(self, y, t, percussion, offset_measure, h, *, endpoint):
        ...
        for k, ci in enumerate(self.contacts):
            ...
            effective_blk = self._storage_to_reported(percussion[sl], sl) + offset_measure[sl]
            if velocity_normal and gaps[k] > self.gap_tol:
                # Moreau viability: open contact is inactive (see residual).
                Jr[sl, sl] = np.diag(self._storage_jacobian_scale(sl))
                continue
            if velocity_normal:
                gap_or_xi = float(u_contact[sl.start])
