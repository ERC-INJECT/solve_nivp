...
cs_ds = build_dynamic_desaxce_residual_contact(
    A=E_D,
...
n_aug_total = cs_ds.y0.size
nl_atol_ds = np.full(n_aug_total, 1e-8)
nl_rtol_ds = np.full(n_aug_total, 1e-6)
nl_atol_ds[ndofs_aug:] = 1e-10
nl_rtol_ds[ndofs_aug:] = 0.0

solver_opts_ds = dict(cs_ds.solver_opts) if cs_ds.solver_opts else {}
solver_opts_ds.update(
    tol=1e-6,
    max_iter=500,
    rhs_jac=cs_ds.rhs_jac,
    linear_solver='petsc',
    globalization='none',
)

integrator_opts_ds = dict(cs_ds.integrator_opts) if cs_ds.integrator_opts else {}
if METHOD == 'radau_iia':
    _use_cn = bool(int(__import__('os').environ.get('RADAU_CN', '1')))
    integrator_opts_ds.update(stages=2, use_coupled_newton=_use_cn)
...
solve_kwargs = dict(
    ...
    A=cs_ds.A,
    method=METHOD,
    projection=cs_ds.projection,
    solver='semismooth_newton',
    solver_opts=solver_opts_ds,
    nl_atol=nl_atol_ds,
    nl_rtol=nl_rtol_ds,
    component_slices=cs_ds.component_slices,
    integrator_opts=integrator_opts_ds,
    h0=h0_init,
    dae_var_weight='auto',
...
if USE_ADAPTIVE:
    solve_kwargs.update(
        ...
        rtol=5e-3,
        atol=1e-4,
        skip_error_indices=[0, len(cs_ds.component_slices) - 1],
    )
else:
    solve_kwargs.update(adaptive=False)

print(f"\n{'='*64}")
print(f"Running Radau+DeSaxce: h0={h0_init:.4g}, n_steps={n_steps_init}, tmax={tmax}")
print(f"ηs={ηs}, L={L}")
...
start = time.time()
t_ds, y_ds, h_ds_hist, fk_ds, info_ds = solve_nivp.solve_ivp_ns(**solve_kwargs)
time_ds = time.time() - start

mode_label = "adaptive" if USE_ADAPTIVE else "fixed-step"
n_steps_taken = len(t_ds) - 1
h_arr = np.asarray(h_ds_hist, dtype=float) if h_ds_hist is not None else np.array([])
print(f"\nRadau+DeSaxce ({mode_label}) elapsed: {time_ds:.2f} s")
...
print(f"  reached t = {float(t_ds[-1]):.6g}")
...
for si in sample_steps:
    ...
    print(f"  {si:5d} {t_ds[si]:8.4f} {v2_val:14.6e} {u2_val:14.6e} {v1_val:14.6e} {p_val:14.6e} {f_elastic_v2:12.4e} {f_elastic_v1:12.4e}")

# Check for NaN/Inf
if np.any(~np.isfinite(y_ds)):
    print("\n*** WARNING: NaN/Inf found in solution! ***")
    nan_steps = np.where(~np.all(np.isfinite(y_ds), axis=1))[0]
    print(f"  First NaN/Inf at step index: {nan_steps[0] if len(nan_steps) else 'none'}")
    if len(nan_steps):
        print(f"  t at that step: {t_ds[nan_steps[0]]:.6g}")

# Check convergence info
if isinstance(info_ds, (list, tuple)) and len(info_ds) > 0:
    if isinstance(info_ds[0], tuple):
        converged = [s[1] for s in info_ds]
        iters = [s[2] for s in info_ds]
...
for j in range(max(0, fail_step-5), min(len(iters), fail_step+2)):
    print(f"  step {j}: t={t_ds[min(j+1,len(t_ds)-1)]:.5f}, iters={iters[j]}, ok={converged[j]}")
